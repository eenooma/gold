...
add_shortcode('myview', 'my_board_view');
function my_board_view($args) {
	$content = new KBContent('4');
	$content->initWithUID($args['uid']);

	ob_start();
	?>
	<article class="common_table <?php echo $args['uid']=='14'?'scrap_gold':'gold'?>">
		<div class="text_wrap">
			<ul>
				<li class="title"><?php echo $args['uid']=='14'?'<strong>폐금</strong>매입시세':'<strong>금</strong> 매입시세'?></li>
				<li class="date"><?php echo $content->option->row['date']?></li>
			</ul>
		</div>
		...
		<div class="table">
			<ul>
				<li>
					...
					<span class="yesterday"><h3>전일시세</h3></span>
					<span class="today"><h3>오늘시세</h3></span>
				</li>
				<?php if($content->option->row['key1']): ?>
				<li>
